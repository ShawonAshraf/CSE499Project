...
log("[INFO] pixels matrix: {:.2f}MB".format(
	rawImages.nbytes / (1024 * 1000.0)))
log("[INFO] features matrix: {:.2f}MB".format(
...
# k-NN
print("\n")
log("[INFO] evaluating raw pixel accuracy...")
model = KNeighborsClassifier(n_neighbors=args["neighbors"])
model.fit(trainRI, trainRL)
acc = model.score(testRI, testRL)
log("[INFO] k-NN classifier: k=%d" % args["neighbors"])
log("[INFO] raw pixel accuracy: {:.2f}%".format(acc * 100))

# k-NN
log("\n")
log("[INFO] evaluating histogram accuracy...")
model = KNeighborsClassifier(n_neighbors=args["neighbors"])
model.fit(trainFeat, trainLabels)
acc = model.score(testFeat, testLabels)
log("[INFO] k-NN classifier: k=%d" % args["neighbors"])
log("[INFO] histogram accuracy: {:.2f}%".format(acc * 100))
...
model.fit(trainRI, trainRL)
acc = model.score(testRI, testRL)
log("[INFO] neural network raw pixel accuracy: {:.2f}%".format(acc * 100))
...
model.fit(trainFeat, trainLabels)
acc = model.score(testFeat, testLabels)
log("[INFO] neural network histogram accuracy: {:.2f}%".format(acc * 100))
